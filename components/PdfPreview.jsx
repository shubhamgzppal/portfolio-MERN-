...
export default function PdfPreview({ fileUrl, className='' }) {
  ...
  const onDocumentLoadError = (error) => { console.error('PdfPreview load error for', fileUrl, 'workerSrc=', pdfjs.GlobalWorkerOptions.workerSrc, error);
    setLoadError(error?.message || String(error));
  };

  useEffect(() => { function updateWidth() { if (containerRef.current) setContainerWidth(containerRef.current.clientWidth);}
    updateWidth();
  ...
  return (
    ...
    <div className="flex justify-center items-center gap-4 mt-4">
        ...
        <button onClick={() => setScale(s => +(s + 0.1).toFixed(2))} className="px-3 py-1 rounded bg-gray-600 text-white" aria-label="Zoom in">
          +
        </button>
      </div>
      ...
      {numPages && numPages > 1 && (
        <div className="flex justify-center items-center gap-4 mt-2">
          <button onClick={() => setPageNumber(prev => Math.max(prev - 1, 1))} disabled={pageNumber === 1} className="px-3 py-1 rounded bg-secondary text-white disabled:opacity-50">
            Previous
          </button>
          <span className="text-sm text-gray-300">Page {pageNumber} of {numPages}</span>
          <button onClick={() => setPageNumber(prev => Math.min(prev + 1, numPages))} disabled={pageNumber === numPages} className="px-3 py-1 rounded bg-secondary text-white disabled:opacity-50">
            Next
          </button>
        </div>
      )}
