...
const Chatbot = ({ isOpen, onClick }) => {
  ...
  return (
    <>
      ...
      {/* Chat Window */}
      <div
        className={`fixed bottom-1 right-1 w-[90vw] sm:w-[30vw] max-w-sm bg-white border border-gray-200 rounded-xl shadow-2xl z-50 flex flex-col transition-all duration-300 ease-in-out
        ${isOpen ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10 pointer-events-none'}`}
      >
        <div className="flex justify-between items-center bg-gray-100 text-gray-800 px-4 py-3 rounded-t-xl border-b">
          <div className="flex items-center space-x-2">
            <div className="w-3 h-3 bg-green-500 rounded-full" />
            <span className="font-semibold">Shubham’s Assistant</span>
          </div>
          <button onClick={onClick} className="text-gray-600 hover:text-gray-800 text-2xl">&times;</button>
        </div>
        ...
        <div className="p-3 bg-gray-100 border-t flex items-center rounded-b-xl">
          <input
            type="text"
            value={input}
          ...
          <button
            onClick={sendMessage}
            disabled={loading}
            className="ml-2 px-4 py-2 bg-blue-600 text-white rounded-full hover:bg-blue-700 disabled:bg-blue-400"
          >
            Send
          </button>
        </div>
