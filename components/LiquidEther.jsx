...
`;
    ...
    // ensure the mount has a usable layout so renderer sizing works
    container.style.position = container.style.position || 'relative';
    container.style.overflow = container.style.overflow || 'hidden';
    try {
      const rect = container.getBoundingClientRect();
      if (!rect || rect.width === 0 || rect.height === 0) {
        // fallback to viewport sized mount so the canvas gets a real size
        container.style.width = '100vw';
        container.style.height = '100vh';
        container.style.minHeight = '100vh';
        // keep it behind content if CSS didn't already do so
  ...
  return () => {
      ...
      if (webglRef.current) {
        console.debug('[LiquidEther] disposing webgl');
